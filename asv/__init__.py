...
for benchmark in glob.glob('**/benchmark/'):
    os.system("echo 'Processing %s'"%benchmark)
    os.chdir(benchmark)
    subprocess.check_call(["make", "benchmark"])
    subprocess.check_call(["./benchmark", "--benchmark_format=json", "--benchmark_out=benchmark.json"])
    results = json.load(open("benchmark.json", "r"))
